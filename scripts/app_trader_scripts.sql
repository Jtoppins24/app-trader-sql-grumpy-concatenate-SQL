...
-- 	(ROUND(ROUND(apple.rating + play.rating)/2, 1)*2)+1 AS life_span,
-- 	apple.price AS app_store_price, 
-- 	CAST(TRIM('$' FROM play.price) AS numeric) as play_store_price
-- 	GREATEST(apple.price, CAST(TRIM('$' FROM play.price) AS numeric)) AS cost,
-- FROM app_store_apps as apple
-- INNER JOIN play_store_apps AS play
-- 	USING(name)
-- 	WHERE apple.price <= 1
-- 	AND CAST(TRIM('$' FROM play.price) AS numeric) <= 1
